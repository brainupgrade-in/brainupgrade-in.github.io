 ...
 *
 * Why this exists at all: the twin API on health.gheware.com took 359 bot rows
 * (Faker names + harvested real emails) posted straight at it; this one took 6
 * list-bombed "newsletter" signups from one IP. dev-leads.gheware-ai.com now
 * verifies a token on EVERY endpoint, not just the proposal form.
 *
 * Loaded by /js/template-loader.js on every post and by index.html directly.
 *
 * Requires, before this file:
 *   <script src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" async defer></script>
 */
(function () {
  var SITE_KEY = '0x4AAAAAAC0hm97i76jCjKVS';
  var API_RE = /^https:\/\/dev-leads\.gheware-ai\.com\//;
  ...
  function render(form) {
    ...
    if (form.querySelector('.cf-turnstile, [name="cf-turnstile-response"]')) return; // page renders its own
    ...
    entry.id = window.turnstile.render(holder, {
      sitekey: SITE_KEY,
      appearance: 'interaction-only',   // inline flex forms: stay invisible unless a challenge is needed
